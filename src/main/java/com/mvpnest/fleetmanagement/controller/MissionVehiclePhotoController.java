...
public class MissionVehiclePhotoController {
    ...
    @GetMapping("/{id}")
    public MissionVehiclePhotoDTO getById(@PathVariable UUID id) {
    ...
    public List<MissionVehiclePhotoDTO> getByInspection(@PathVariable UUID inspectionId) {
    ...
    @PostMapping("/upload")
    public MissionVehiclePhotoDTO upload(@RequestPart("file") MultipartFile file, @RequestParam UUID inspectionId, @RequestParam(required = false) String description) {

        return service.uploadPhoto(file, inspectionId, description);
