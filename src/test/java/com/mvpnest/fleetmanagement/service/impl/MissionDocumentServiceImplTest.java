...
class MissionDocumentServiceImplTest {
    ...
    void uploadDocument_withUploader_savesDocument() {
        ...
        ReflectionTestUtils.setField(missionDocumentService, "uploadDir", tempDir.toString());

        UUID missionId = UUID.randomUUID();

        Mission mission = Mission.builder().id(missionId).title("Tunis Delivery").build();
        ...
        when(missionDocumentRepository.save(any(MissionDocument.class))).thenAnswer(invocation -> invocation.getArgument(0));

        when(mapper.toDTO(any(MissionDocument.class))).thenReturn(new MissionDocumentDTO());

        missionDocumentService.uploadDocument(file, request, user);

        verify(missionDocumentRepository).save(argThat(doc -> doc.getUploadedBy().equals(user)));
    }
}